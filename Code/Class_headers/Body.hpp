...
namespace DeltaEngine
{
	...
	public:
		...
		void set_angle(int angle, bool inRad = true);
		void set_displayScreen(uint ds);

		///Basic func.
		bool load(); //Load the stats of the body by a file. Return the success.
		void move(double time); //Move the body with its velocity and its forces/impulses.
		...
		///Physic func.
		Vec2f moveTest(double time) const; //Return the future velocity without modifying the body stats.
		void applyForce(Vec2f force); //Apply a force to the body.
		void applyImpulse(Vec2f force); //Apply an impulse to the body.
	...
	protected:
		///Basic Members
		Vertex m_pos; //The position of the body.
		Vertex /*Not sure about useness.*/ m_center; //The center of the body.
		double m_angle; //The angle of the body.
		uint m_displayScreen; //The DS where the body is.

		///Render Members
		uint m_indexSSMng, m_indexTexMng; //The index of the body for the Managers.
		...
		uint m_frameSS, m_frameTex; //(AF) The current Frame of the Animation.
		Vertex m_defaultFrameSS, m_defaultFrameTex; //(DefF) The DEFault Frame displayed.

		///Physic Members
		ulong m_mass, m_density, m_friction, m_restitution; //For speed moving, evolution in fluids, tailback or bouncing.
		Vec2f m_force{ 0, 0 }, m_impule{ 0, 0 }, m_velocity{ 0, 0 }; //Forces stay after implementation, impulses disappear.
		//Both increase the velocity of the body.
		moveType m_moveType; //Define if the body can move, and how it can.
		collisionType m_collisionType; //To define which collides with it.
